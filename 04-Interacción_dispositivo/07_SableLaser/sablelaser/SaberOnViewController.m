...
- (void)loadView {
    ...
    [acelerometro setAccelerometerUpdateInterval:0.1];
    
    
    [acelerometro startAccelerometerUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMAccelerometerData *accelerometerData, NSError *error) {
       
        if(error != nil) {
            NSLog(@"Error");
        }
        else {
            [self accelerometerOutput:accelerometerData.acceleration];
        }
...
- (void)accelerometerOutput:(CMAcceleration)accel
{
